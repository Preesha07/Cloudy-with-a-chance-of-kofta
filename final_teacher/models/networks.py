...
def define_A(input_nc, output_nc, ngf, which_model_netA, norm='batch', use_dropout=False, init_type='normal', gpu_ids=[], ste=False):
    netA = None
    use_gpu = len(gpu_ids) > 0
    norm_layer = get_norm_layer(norm_type=norm)
    ...
    norm_cls = NormSTE if ste else Norm
    netA = nn.Sequential(netA, norm_cls())
    if len(gpu_ids) > 0:
        # netA.cuda(device_id=gpu_ids[0])
        netA.cuda(gpu_ids[0])
...
class _NormSTEFn(torch.autograd.Function):
    """clamp(abs(x), 0, 1) forward; straight-through (identity) backward.

    The standard Norm kills gradients wherever abs(x) saturates at 0 or 1.
    With STE the backward pretends the op is the identity: gradients flow
    through unchanged, so the upstream attention network keeps learning even
    when individual map values are pinned at the boundaries.
    """
    @staticmethod
    def forward(ctx, x):
        return torch.clamp(torch.abs(x), 0.0, 1.0)

    @staticmethod
    def backward(ctx, grad_output):
        return grad_output   # identity — no clamp/abs on the gradient

class NormSTE(nn.Module):
    """Drop-in replacement for Norm that uses a straight-through estimator."""
    def forward(self, x):
        return _NormSTEFn.apply(x)
