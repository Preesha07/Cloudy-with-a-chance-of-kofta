...
def dist_intra_loss(t_act, s_act, min_spatial=8, eps=1e-8):
    ...
    When the spatial grid is smaller than min_spatial x min_spatial, both maps
    are upsampled via bilinear interpolation before computing Pearson.  Upsampling
    BOTH teacher and student by the same factor preserves the relative spatial
    pattern (which quadrant fires vs which doesn't) while giving Pearson enough
    samples to be a stable estimator.  At block 7 (2x2 -> 8x8) N goes from 4 to
    64, dropping Pearson standard error from ~0.7 to ~0.12.

    This is valid only for within-depth comparison (same block, teacher vs student).
    It differs from FSP's "never upsample" rule, which applies cross-depth where
    upsampling would invent spatial detail at the wrong resolution.
    """
    h, w = t_act.shape[2], t_act.shape[3]
    if h < min_spatial or w < min_spatial:
        t_act = F.interpolate(t_act, size=(min_spatial, min_spatial),
                              mode='bilinear', align_corners=False)
        s_act = F.interpolate(s_act, size=(min_spatial, min_spatial),
                              mode='bilinear', align_corners=False)
    t = _flatten_to_instances(t_act)
    s = _flatten_to_instances(s_act)
...
class Pix2Pix_attn_Student_FSP_Model(BaseModel):
    ...
    def initialize(self, opt):
        ...
        # Minimum spatial side length for L_intra.  When a block's H or W is
        # smaller than this, both teacher and student activations are bilinearly
        # upsampled to this size before computing Pearson.  Default 8 gives N=64
        # at block 7 (2x2) instead of N=4, reducing Pearson std error from ~0.7
        # to ~0.12.  Set to 0 or 1 to disable upsampling entirely.
        self.dist_intra_min_spatial = getattr(opt, 'dist_intra_min_spatial', 8)
        ...
        if self.isTrain:
            ...
            # netA is frozen: taken directly from the teacher, never updated.
            ...
            if hasattr(opt, 'hook_layers') and opt.hook_layers:
                ...
                self.hook_layer_indices = list(range(3, n_blocks - 1))
            ...
            print('DIST: beta(inter)=%g  gamma(intra)=%g  act=%s  intra_min_spatial=%d'
                  % (self.dist_beta, self.dist_gamma, self.dist_act,
                     self.dist_intra_min_spatial))
            ...
            # Continue-train: restore student weights (not netA — it stays frozen from teacher)
        ...
        if self.isTrain:
            ...
            self.optimizers  = [self.optimizer_G, self.optimizer_H, self.optimizer_D]
    ...
    def forward(self):
        ...
        # Attention from frozen teacher netA — no grad needed
        with torch.no_grad():
    ...
    def _hallucination_loss(self):
        ...
        total = 0.0
        for idx in self.hook_layer_indices:
            t_act = self._out_half(self._teacher_acts[idx], idx)   # detached → stable supervision
            s_act = self._out_half(self._student_acts[idx], idx)   # differentiable → grad into netH
    ...
    def _dist_losses(self):
        ...
        L_intra is computed at every hooked block including the deep bottleneck
        blocks (6 and 7).  For blocks whose spatial grid is smaller than
        dist_intra_min_spatial x dist_intra_min_spatial, dist_intra_loss
        bilinearly upsamples both teacher and student activations before computing
        Pearson, so the correlator sees enough samples to be statistically stable.
        ...
        for idx in self.hook_layer_indices:
            ...
            l_intra = l_intra + dist_intra_loss(t_act, s_act,
                                                min_spatial=self.dist_intra_min_spatial,
                                                eps=self.dist_eps)
    ...
    def _freqkd_layer_loss(self, t_act, s_act):
        ...
        mask = self._get_radial_mask(H, W, t_act.device).unsqueeze(0).unsqueeze(0)
        ...
        l_low  = (s_low  - t_low ).pow(2).mean()
        l_high = torch.log(1.0 + (s_high - t_high).pow(2).mean())
        return l_low, l_high
    ...
    def _freqkd_loss(self):
        ...
        for idx in self.hook_layer_indices:
            ...
            acc[1] += l_high.item()
            acc[2] += 1
            total = total + l_low + self.freqkd_eta * l_high
        return total
    ...
    def get_epoch_metrics(self):
        """Per-pair mean distance and degenerate fraction over the epoch.

        This is the instrument for judging whether the teacher's FSP matrices
        carry real cross-depth structure -- particularly at pairs 5-6 and 6-7,
        where rank(G) is capped at 16 and 4 by the pooled position count.
        """
        out = OrderedDict()
    ...
    def _compute_losses(self):
        ...
        # ── L_G: adversarial + L1 + SSIM (no sparsity — netA is frozen) ──────
    ...
    def optimize_parameters(self):
        ...
        # ── Combined student backward (G + H) — netA is frozen, not updated ────
