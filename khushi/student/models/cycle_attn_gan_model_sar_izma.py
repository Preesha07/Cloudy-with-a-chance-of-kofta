...
class CycleAttnGANModel_sar(BaseModel):
    def name(self):
    ...
    def initialize(self, opt):
        ...
        self.input_C = self.Tensor(nb, opt.input_nc, size, size)         #SAR
        ...
        self.netG_A = networks.define_G(opt.input_nc+opt.input_nc, opt.output_nc,
        ...
        print('-----------------------------------------------')
        # Epoch-level metric accumulators — reset at the start of each epoch
        self._epoch_psnr = 0.0
        self._epoch_ssim = 0.0
        self._epoch_mae  = 0.0
        self._epoch_attn = 0.0
        self._epoch_n    = 0
        # Per-batch metrics initialised to safe defaults so get_current_errors()
        # can be called before the first backward_G() (e.g. during visualisation)
        self.psnr_fakeB = 0.0
        self.ssim_fakeB = 0.0
        self.mae_fakeB  = 0.0
        self.attn_mean  = 0.0
    ...
    def backward_G(self):
        ...
        if lambda_idt > 0:
            ...
            self.idt_A = self.netG_A.forward(torch.cat([self.real_B,self.real_C],dim = 1))
        ...
        #fake_B = self.netG_A.forward(self.real_A+self.real_C)
        fake_B = self.netG_A.forward(self.real_AC)
        ...
        self.ssimloss1 = 1 - ssim_loss(self.fake_B, self.real_B)

        # Quality metrics — detached so they carry no gradient overhead.
        # psnr_fakeB: primary cloud-removal quality (dB, higher = better, ~20–35 is good range)
        # ssim_fakeB: perceptual similarity (0–1, higher = better)
        # mae_fakeB:  pixel-level error in [-1,1] space (lower = better)
        # attn_mean:  mean attention activation — watch for collapse toward 0 (sparsity too strong)
        with torch.no_grad():
            _mse = torch.mean((self.fake_B.detach() - self.real_B.detach()) ** 2)
            self.psnr_fakeB = (20.0 * torch.log10(
                torch.tensor(2.0, device=_mse.device) / (torch.sqrt(_mse) + 1e-8)
            )).item()
            self.ssim_fakeB = (1.0 - self.ssimloss1).item()
            self.mae_fakeB = torch.mean(torch.abs(self.fake_B.detach() - self.real_B.detach())).item()
            self.attn_mean = self.attn_real_A.detach().mean().item()

        #self.loss_G_A = self.criterionGAN(pred_fake, True)+self.criterionIdt(self.fake_B,self.real_B)
        self.loss_G_A = self.criterionGAN(pred_fake, True)+self.ssimloss1
        ...
        self.ssimloss2 = 1 - ssim_loss(self.fake_A,self.real_A)
        #self.loss_G_B = self.criterionGAN(pred_fake, True)+self.criterionIdt(self.fake_A,self.real_A)
        self.loss_G_B = self.criterionGAN(pred_fake, True)+self.ssimloss2
        # Forward cycle loss
        rec_A = self.netG_B.forward(self.fake_B)
        self.attn_fake_B = self.netA_B.forward(self.fake_B)
        self.rec_A = self.mask_layer(rec_A, self.fake_B, self.attn_fake_B)
        ...
        rec_B = self.netG_A.forward(torch.cat([self.fake_A,self.real_C],dim = 1))
        self.attn_fake_A = self.netA_A.forward(self.fake_A)
        self.rec_B = self.mask_layer(rec_B, self.fake_A, self.attn_fake_A)
    ...
    def get_current_errors(self):
        ...
        # ssim_A/B are 1-SSIM loss terms (lower = better); psnr/ssim/mae are true metrics
        ssim_A = self.ssimloss1.data
        ssim_B = self.ssimloss2.data
        quality = [
            ('psnr', self.psnr_fakeB),   # dB — primary reconstruction metric (↑ better)
            ('ssim', self.ssim_fakeB),   # 0–1 — perceptual quality (↑ better)
            ('mae',  self.mae_fakeB),    # pixel error in [-1,1] (↓ better)
            ('attn', self.attn_mean),    # diagnostic: near-0 means SAR edits suppressed
        ]
    ...
    def accumulate_epoch_metrics(self):
        self._epoch_psnr += self.psnr_fakeB
        self._epoch_ssim += self.ssim_fakeB
        self._epoch_mae  += self.mae_fakeB
        self._epoch_attn += self.attn_mean
        self._epoch_n    += 1

    def get_epoch_metrics(self):
        n = max(self._epoch_n, 1)
        return OrderedDict([
            ('psnr', self._epoch_psnr / n),
            ('ssim', self._epoch_ssim / n),
            ('mae',  self._epoch_mae  / n),
            ('attn', self._epoch_attn / n),
        ])
