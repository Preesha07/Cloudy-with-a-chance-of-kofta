...
class TeacherWrapper(nn.Module):
    ...
    def __init__(self, teacher_model):
        ...
        # Pix2Pix_attn_Model is a framework wrapper, not nn.Module — freeze its sub-networks.
        for net_name in ("netG", "netG2", "netA", "netD"):
            net = getattr(teacher_model, net_name, None)
            if net is not None:
                net.eval()
...
def train_student_gan(student_G, student_D, teacher, dataloader, projections, cfg=CFG,
    ...
    if resume is not None:
        ...
        student_G.load_state_dict(ckpt["student_G"])
        student_D.load_state_dict(ckpt["student_D"])
        projections.load_state_dict(ckpt["projections"])
        opt_G.load_state_dict(ckpt["opt_G"])
        opt_D.load_state_dict(ckpt["opt_D"])
        start_epoch = ckpt["epoch"] + 1
        print(f"[train_student_gan] resumed from epoch {ckpt['epoch']} → starting at epoch {start_epoch}")
