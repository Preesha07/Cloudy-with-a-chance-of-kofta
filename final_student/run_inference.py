...
Usage (see final_student/README.md, or scripts/run_inference.sh):
...
# Allow running from anywhere — everything resolves inside final_student/.
...
from pipeline.model import load_student_model, apply_ttbn, run_adabn, try_compile
from pipeline.stitch import (
    Stitcher,
    read_manifest,
    get_shear_params,
...
from methods import get_adapter
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--method", required=True,
                   help="Adaptation method. Only dataset_norm ships in final_student/ "
                        "(it is what the final runs used); the exploratory adapters "
                        "live in domain_adapt/methods/.")
    ...
    p.add_argument("--ref_image", default=None,
                   help="Path to a SEN12MS-CR cloud-free reference image (RGB, 256x256 OK). "
                        "Unused by dataset_norm; kept for adapters that need a reference.")
    p.add_argument("--ref_root", default=None,
                   help="Path to SEN12MSCR_student/ root. Unused by dataset_norm.")
...
def main():
    args = parse_args()
    adapter = get_adapter(args.method, ref_root=getattr(args, 'ref_root', None))

    if adapter.needs_ref_image and args.ref_image is None:
        sys.exit(f"ERROR: --method {args.method} requires --ref_image")

    # Load model
    netG, netA, netH, device = load_student_model(args.weights)
    for net in (netG, netA, netH):
        apply_ttbn(net)
    netG, netA, netH = try_compile(netG, netA, netH)

    ref_tensor = load_ref_tensor(args.ref_image, device)
